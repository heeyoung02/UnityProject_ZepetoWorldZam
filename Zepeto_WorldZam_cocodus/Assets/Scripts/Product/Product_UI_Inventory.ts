...
import ITM_Inventory1 from './ITM_Inventory1';
import {BalanceSync, InventorySync, Currency} from "./Product_UI_Balance";

export default class Product_UI_Inventory extends ZepetoScriptBehaviour {
    ...
    private UpdateInventory(items:InventoryRecord[]){
        const itemScripts = this.contentParent.GetComponentsInChildren<ITM_Inventory1>();
        items.forEach((ir)=>{
    ...
    private CreateInventory(items :InventoryRecord[]){
        this.contentParent.GetComponentsInChildren<ITM_Inventory1>().forEach((child)=>{
        ...
        items.forEach((ir, index) => {
            ...
            const itemScript = itemObj.GetComponent<ITM_Inventory1>();
    ...
    private OnClickUseInventoryItem(){
        ...
        const item = toggleGroup.GetFirstActiveToggle()?.GetComponent<ITM_Inventory1>().itemRecord;
